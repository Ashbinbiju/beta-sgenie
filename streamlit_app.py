...
def detect_divergence(data):
    ...
    rsi = data['RSI']
    price = data['Close']
    lookback = 10  # Extended lookback period
...
def calculate_heikin_ashi(data):
    try:
        ...
        ha_open = ((data['Open'].shift(1) + data['Close'].shift(1)) / 2).fillna(data['Open'].iloc[0])
        ha_data['HA_Open'] = ha_open.astype(np.float32)

        ha_data['HA_High'] = np.maximum.reduce([
            data['High'], ha_data['HA_Open'], ha_data['HA_Close']
        ]).astype(np.float32)

        ha_data['HA_Low'] = np.minimum.reduce([
            data['Low'], ha_data['HA_Open'], ha_data['HA_Close']
        ]).astype(np.float32)
...
def calculate_adaptive_volume_spike(data, window=20):
    try:
        ...
        if not pd.isna(atr.iloc[-1]) and data['Close'].iloc[-1] != 0:
            volatility_factor = atr.iloc[-1] / data['Close'].iloc[-1]
        else:
            volatility_factor = 1
        ...
        # Return boolean series for volume spikes
...
def train_ml_model(data, indicators):
    try:
        # Filter features based on the provided indicators
        ...
        labels = (returns > 0).astype(int)  # 1 for up, 0 for down
        features = features.loc[labels.index]

        # Ensure sufficient data after matching labels
        if len(features) < 20:
            logging.warning("Insufficient data after matching features and labels")
            return None
        ...
        logging.info(f"ML model trained with accuracy: {accuracy:.2f}, CV mean: {cv_scores.mean():.2f}")

        # If the model's CV score is too low, return None
        if cv_scores.mean() < 0.6:
            logging.warning("ML model has low cross-validation score")
            return None

        # Return the trained model and selected features
...
def update_indicator_weights(data, indicators):
    ...
    try:
        # Train the model and retrieve selected features
        model, selected_features = train_ml_model(data, indicators)

        # If model training fails, return existing weights
        if model is None:
            logging.warning("Model training failed, returning existing indicator weights")
            return INDICATOR_WEIGHTS

        # Get feature importances from the trained model
        importances = model.feature_importances_

        # Initialize the new weights dictionary
        ...
        for i, feature in enumerate(selected_features):
            if feature in new_weights:
                # Scale the importance between 0.5 and 2.0
                new_weights[feature] = min(2.0, max(0.5, importances[i] * 2))

        # Update the global indicator weights
...
def get_sector(symbol):
    # Iterate over sectors and their respective stocks
    for sector, stocks in SECTORS.items():
        # Check if symbol is in the list of stocks for this sector
        if symbol in stocks:
            return sector
    # Return None if symbol is not found in any sector
    return None
...
def analyze_stock(data, indicators=None, symbol=None):
    ...
    all_indicators = [
        ...
        'CMF', 'Donchian', 'Keltner', 'TRIX', 'Ultimate_Osc', 'CMO', 'VPT', 'Pivot', 'Heikin-Ashi'
    ]

    indicators = all_indicators if indicators is None else indicators
    computed_indicators = []
    indicator_order = ['ATR', 'RSI', 'MACD', 'SMA_EMA', 'Bollinger', 'Stochastic', 'ADX', 'OBV', 'VWAP', 
                       'Volume_Spike', 'Parabolic_SAR', 'Fibonacci', 'Divergence', 'Ichimoku', 'CMF', 'Donchian', 
                       'Keltner', 'TRIX', 'Ultimate_Osc', 'CMO', 'VPT', 'Pivot', 'Heikin-Ashi']
    ...
    indicator_functions = {
        ...
        'SMA_EMA': lambda: calculate_sma_ema(data),
        'Bollinger': lambda: calculate_bollinger(data),
        'Stochastic': lambda: calculate_stochastic(data),
        'ATR': lambda: calculate_atr(data),
        'ADX': lambda: calculate_adx(data),
        'OBV': lambda: calculate_obv(data),
        'VWAP': lambda: calculate_vwap(data),
        'Volume_Spike': lambda: calculate_volume_spike(data),
        'Parabolic_SAR': lambda: calculate_parabolic_sar(data),
        'Fibonacci': lambda: calculate_fibonacci(data),
        'Divergence': lambda: calculate_divergence(data),
        'Ichimoku': lambda: calculate_ichimoku(data),
        'CMF': lambda: calculate_cmf(data),
        'Donchian': lambda: calculate_donchian(data),
        'Keltner': lambda: calculate_keltner(data),
        'TRIX': lambda: calculate_trix(data),
        'Ultimate_Osc': lambda: calculate_ultimate_osc(data),
        'CMO': lambda: calculate_cmo(data),
        'VPT': lambda: calculate_vpt(data),
        'Pivot': lambda: calculate_pivot(data),
