...
def get_bullish_sectors():
    """Stub - brkpoint API removed. Returns empty list."""
    return []
...
def calculate_swing_score(df, symbol=None, timeframe='1d', contrarian_mode=False):
    ...
    # Final score (pure technical — no external context)
    final_score = score
    
    # Normalization: technical-only range ~±13
    max_expected = 13
...
def calculate_intraday_score(df, symbol=None, timeframe='15m', contrarian_mode=False):
    ...
    # Normalization (technical-only range ~±18)
    max_expected = 18 if not contrarian_mode else 15
    if final_score >= 0:
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Analysis", "🔍 Scanner", "💰 Paper Trading"])
    ...
    with tab2:
        ...
        with col1:
            if "All" in selected_sectors:
                st.info(f"📊 Scanning {len(stock_list)} stocks from all sectors")
            else:
                st.info(f"📊 Scanning {len(stock_list)} stocks from {len(selected_sectors)} selected sectors: {', '.join(selected_sectors[:3])}{'...' if len(selected_sectors) > 3 else ''}")
